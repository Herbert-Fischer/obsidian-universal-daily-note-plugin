...
This repository is mounted at **`/workspace/universal-daily-note-plugin`**. Each Obsidian plugin stays its own git clone; only this repo is the Dev Container **workspace folder**.
...
is bind-mounted to **`/workspaces/SoftwareEntwicklung`** in the container (note **`workspaces`** with an **`s`** — not **`/workspace/...`**). The image also provides a symlink **`/workspace/SoftwareEntwicklung`** → that path so it sits next to **`/workspace/universal-daily-note-plugin`**. Rebuild the dev container image after this Dockerfile change; until then, use **`cd /workspaces/SoftwareEntwicklung`** directly.
